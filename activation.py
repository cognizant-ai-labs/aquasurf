...
class ActivationFunction(Layer):
    ...
    def construct_function(self, fn_name):
        if fn_name == 'x':
            ...
            op = FUNCTIONS[op_name]
            args = [self.construct_function(arg_name) for arg_name in arg_names]

            if op_name in N_ARY_FUNCTIONS:
                return lambda x : op(tf.stack([arg(x) for arg in args]))
            elif op_name in BINARY_FUNCTIONS:
                assert len(args) == 2
                return lambda x : op(args[0](x), args[1](x))
            else:
    ...
    def build(self, input_shape):
        ...
        if self.normalize:
            gm = gaussian_mean(callable_fn)
            centered_fn = lambda x : callable_fn(x) - gm
            sm = second_moment(centered_fn)
            normalized_fn = lambda x : centered_fn(x) / np.sqrt(sm)
